...
const actions = {
	...
	getUsers(context, { offset, limit, search, group }) {
		...
		if (group !== '') {
			return api.get(generateOcsUrl(`cloud/groups/${encodeURIComponent(encodeURIComponent(group))}/users/details?offset=${offset}&limit=${limit}&search=${search}`), {
	...
	getGroups(context, { offset, limit, search }) {
		...
		return api.get(generateOcsUrl(`cloud/groups?offset=${offset}&search=${search}${limitParam}`))
			.then((response) => {
	...
	getUsersFromGroup(context, { groupid, offset, limit }) {
		return api.get(generateOcsUrl(`cloud/users/${encodeURIComponent(encodeURIComponent(groupid))}/details?offset=${offset}&limit=${limit}`))
	...
	addUserGroup(context, { userid, gid }) {
		return api.requireAdmin().then((response) => {
			return api.post(generateOcsUrl(`cloud/users/${userid}/groups`), { groupid: gid })
	...
	removeUserGroup(context, { userid, gid }) {
		return api.requireAdmin().then((response) => {
			return api.delete(generateOcsUrl(`cloud/users/${userid}/groups`), { groupid: gid })
	...
	addUserSubAdmin(context, { userid, gid }) {
		return api.requireAdmin().then((response) => {
			return api.post(generateOcsUrl(`cloud/users/${userid}/subadmins`), { groupid: gid })
	...
	removeUserSubAdmin(context, { userid, gid }) {
		return api.requireAdmin().then((response) => {
			return api.delete(generateOcsUrl(`cloud/users/${userid}/subadmins`), { groupid: gid })
	...
	setUserData(context, { userid, key, value }) {
		...
		if (['email', 'language', 'quota', 'displayname', 'password'].indexOf(key) !== -1) {
			...
			if (typeof value === 'string'
				...
				return api.requireAdmin().then((response) => {
					return api.put(generateOcsUrl(`cloud/users/${userid}`), { key, value })
